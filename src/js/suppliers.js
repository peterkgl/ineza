document.addEventListener("DOMContentLoaded", function () {
  ...
  function renderSuppliers() {
    ...
    paginated.forEach(function (s, index) {
      ...
      var typeVal = escapeHtml(
        s.supplier_type.charAt(0).toUpperCase() + s.supplier_type.slice(1),
      );
      ...
      html +=
        ...
        '<button class="btn-icon-only edit" title="Edit Supplier" data-id="' +
        s.id +
        '">' +
        ...
        "</button>" +
        '<button class="btn-icon-only delete" title="Delete Supplier" data-id="' +
        s.id +
        '">' +
      ...
      btn.addEventListener("click", function () {
        ...
        if (s) {
          ...
          confirmBody.textContent =
            'Are you sure you want to delete the supplier configuration "' +
            s.name +
            '"? This will fail if advances are recorded under this supplier.';
  ...
  if (supplierForm) {
    supplierForm.addEventListener("submit", function (ev) {
      ...
      var active = document.getElementById("supplierActive").checked
        ? "1"
        : "0";
