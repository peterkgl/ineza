document.addEventListener('DOMContentLoaded', function () {
  ...
  if (currencySelect) {
    currencySelect.addEventListener('change', function () {
      ...
      if (val && val !== '') {
        var cur = window.currenciesList.find(function (c) { return c.id == val; });
  ...
  if (productSelect) {
    productSelect.addEventListener('change', function () {
      ...
      if (!lotId && productId) { // Only prefill for new lots
        fetch('lots_api.php?action=get_carryover&product_id=' + productId)
          .then(function (res) { return res.json(); })
          .then(function (result) {
            if (result.success && result.carryover !== null && result.carryover > 0) {
              document.getElementById('lotQtyReceived').value = result.carryover.toFixed(3);
              document.getElementById('lotQtyRemaining').value = result.carryover.toFixed(3);
              carryoverIndicator.style.display = 'block';
              carryoverIndicator.textContent = 'Carried over balance: ' + result.carryover.toFixed(3) + ' kg';
            } else {
  ...
  function getFilteredLots() {
    ...
    return allLots.filter(function (l) {
      ...
      if (query) {
        ...
        matchQuery = lotNoVal.indexOf(query) !== -1 || descVal.indexOf(query) !== -1;
      }
  ...
  function fetchLots() {
    ...
    fetch('lots_api.php?action=list')
      ...
      .catch(function (error) {
        ...
        showAlert(alertPlaceholder, 'error', 'An error occurred while loading lots.');
      });
  }
  ...
  function renderLots() {
    ...
    if (totalItems === 0) {
      lotsList.innerHTML = '<tr><td colspan="9" class="table-empty">' + (searchInput && searchInput.value.trim() ? 'No matching lots found.' : 'No lots configured yet.') + '</td></tr>';
    ...
    paginated.forEach(function (l, index) {
      ...
      var statusLabel = l.status.toUpperCase() === 'OPEN'
        ? '<span class="status-pill pill-green">Open</span>'
        : '<span class="status-pill pill-red">Closed</span>';

      var actionBtns = '';
      actionBtns += '<button class="btn-icon-only view-btn" title="View Details" data-id="' + l.id + '">' +
        '<svg viewBox="0 0 24 24"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>' +
        '</button>';

      if (l.status.toUpperCase() === 'OPEN') {
        actionBtns += '<button class="btn-icon-only edit" title="Edit Lot" data-id="' + l.id + '">' +
          ...
          '</button>';
        actionBtns += '<button class="btn-icon-only close-btn" title="Close Lot" data-id="' + l.id + '">' +
          '<svg viewBox="0 0 24 24"><rect x="3" y="11" width="18" height="11" rx="2" ry="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>' +
          '</button>';
      } else {
        actionBtns += '<button class="btn-icon-only open-btn" title="Re-open Lot" data-id="' + l.id + '">' +
          '<svg viewBox="0 0 24 24"><polyline points="22 4 12 14.01 9 11.01"/><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/></svg>' +
          '</button>';
      }

      actionBtns += '<button class="btn-icon-only delete" title="Delete Lot" data-id="' + l.id + '">' +
      ...
      html += '<tr>' +
        ...
        '<td><span class="code-badge">' + prodName + '</span></td>' +
        '<td>' + date + '</td>' +
        '<td>' + qtyRec + '</td>' +
        '<td>' + qtyRem + '</td>' +
        '<td>' + whName + '</td>' +
        ...
        '<td style="text-align: right;">' +
          '<div class="action-buttons" style="justify-content: flex-end;">' + actionBtns + '</div>' +
        '</td>' +
      '</tr>';
    });
  ...
  function attachRowEventListeners(lots) {
    ...
    deleteBtns.forEach(function (btn) {
      btn.addEventListener('click', function () {
        ...
        if (l) {
          activeDeleteId = id;
          confirmBody.textContent = 'Are you sure you want to delete lot "' + l.lot_number + '"? This will fail if there are recorded purchases, sales, or stock movements associated with it.';
      ...
      btn.addEventListener('click', function () {
        ...
        if (l) {
          ...
          if (l.remaining_quantity > 0) {
            confirmCloseBody.textContent = 'Lot "' + l.lot_number + '" has a remaining stock balance of ' + l.remaining_quantity.toFixed(3) + ' kg. Are you sure you want to close this lot and confirm its stock balances?';
          } else {
            confirmCloseBody.textContent = 'Are you sure you want to close the lot "' + l.lot_number + '"? Once closed, it will become read-only.';
          }
  ...
  function reopenLot(id) {
    ...
    .catch(function (err) {
      ...
      showAlert(alertPlaceholder, 'error', 'An error occurred while reopening the lot.');
    });
  }
  ...
  function showViewDetails(l) {
    ...
    var html = 
      '<div class="detail-row"><span class="detail-label">Lot ID</span><span class="detail-value">' + l.id + '</span></div>' +
      '<div class="detail-row"><span class="detail-label">Lot Number</span><span class="detail-value">' + l.lot_number + '</span></div>' +
      '<div class="detail-row"><span class="detail-label">Product mineral</span><span class="detail-value">' + (l.product_name || '\u2014') + ' (' + (l.product_code || '\u2014') + ')</span></div>' +
      '<div class="detail-row"><span class="detail-label">Supplier entity</span><span class="detail-value">' + (l.supplier_name || '\u2014') + '</span></div>' +
      '<div class="detail-row"><span class="detail-label">Warehouse location</span><span class="detail-value">' + (l.warehouse_name || '\u2014') + '</span></div>' +
      '<div class="detail-row"><span class="detail-label">Opening Date</span><span class="detail-value">' + l.received_date + '</span></div>' +
      '<div class="detail-row"><span class="detail-label">Closing Date</span><span class="detail-value">' + closeDate + '</span></div>' +
      '<div class="detail-row"><span class="detail-label">Status</span><span class="detail-value">' + l.status + '</span></div>' +
      '<div class="detail-row"><span class="detail-label">Opening Balance</span><span class="detail-value">' + l.quantity_received.toFixed(3) + ' kg</span></div>' +
      '<div class="detail-row"><span class="detail-label">Remaining Balance</span><span class="detail-value">' + l.remaining_quantity.toFixed(3) + ' kg</span></div>' +
      '<div class="detail-row"><span class="detail-label">Unit Cost (Base)</span><span class="detail-value">' + uCost + '</span></div>' +
      '<div class="detail-row"><span class="detail-label">Currency</span><span class="detail-value">' + cur + '</span></div>' +
      '<div class="detail-row"><span class="detail-label">Exchange Rate</span><span class="detail-value">' + exRate + '</span></div>' +
      '<div class="detail-row"><span class="detail-label">Description / Notes</span><span class="detail-value">' + desc + '</span></div>' +
      '<div class="detail-row"><span class="detail-label">Configured By</span><span class="detail-value">' + l.created_by_name + '</span></div>' +
      '<div class="detail-row"><span class="detail-label">Created At</span><span class="detail-value">' + l.created_at + '</span></div>';
  ...
  function setEditMode(l) {
    ...
    document.getElementById('lotQtyReceived').value = l.quantity_received.toFixed(3);
    document.getElementById('lotQtyRemaining').value = l.remaining_quantity.toFixed(3);
    document.getElementById('lotUnitCost').value = l.unit_cost !== null ? l.unit_cost.toFixed(2) : '';
    ...
    if (l.exchange_rate) {
      document.getElementById('lotExchangeRate').value = l.exchange_rate.toFixed(6);
    }
  ...
  function resetForm() {
    ...
    document.getElementById('lotOpeningDate').value = new Date().toISOString().substring(0, 10);
    carryoverIndicator.style.display = 'none';
    exchangeRateGroup.style.display = 'none';
  ...
  if (lotForm) {
    lotForm.addEventListener('submit', function (ev) {
      ...
      if (!qtyReceived) {
        showAlert(formAlertPlaceholder, 'error', 'Opening balance quantity is required.');
        return;
      }
      ...
      .catch(function (error) {
        ...
        saveBtn.disabled = false;
        saveBtn.textContent = id ? 'Update Lot' : 'Save Lot';
        showAlert(formAlertPlaceholder, 'error', 'An error occurred while saving the lot.');
      });
    });
  }
  ...
  confirmDeleteBtn.addEventListener('click', function () {
    ...
    .catch(function (err) {
      ...
      showAlert(alertPlaceholder, 'error', 'An error occurred while deleting the lot.');
      activeDeleteId = null;
    });
  });
  ...
  closeConfirmBtn.addEventListener('click', function () {
    ...
    .catch(function (err) {
      ...
      showAlert(alertPlaceholder, 'error', 'An error occurred while closing the lot.');
      activeCloseId = null;
    });
  });
