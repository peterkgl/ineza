  ...
  function renderAccounts() {
    if (!allAccounts || allAccounts.length === 0) {
      accountsList.innerHTML =
        '<tr><td colspan="6" class="table-empty">No accounts registered yet.</td></tr>';
    ...
    paginated.forEach(function (a, index) {
      ...
      html +=
        ...
        globalIndex +
        "</td>" +
        '<td><span class="code-badge" style="font-weight:600; font-family:monospace;">' +
        escapeHtml(a.account_code) +
        "</span></td>" +
        '<td class="td-name" style="font-weight:500;">' +
        escapeHtml(a.account_name) +
        "</td>" +
        '<td><span class="parent-type-badge">' +
        escapeHtml(a.account_type_name) +
        " (" +
        escapeHtml(a.account_type_code) +
        ")</span></td>" +
        "<td>" +
        activeLabel +
        "</td>" +
        ...
        '<button class="btn-icon-only edit" title="Edit Account" data-id="' +
        a.id +
        '">' +
        ...
        "</button>" +
        '<button class="btn-icon-only delete" title="Delete Account" data-id="' +
        a.id +
        '">' +
  ...
  function attachRowEventListeners(accounts) {
    ...
    editButtons.forEach(function (btn) {
      btn.addEventListener("click", function () {
        ...
        var a = accounts.find(function (item) {
          return item.id === id;
        });
        if (a) {
          setEditMode(a);
    ...
    fetch("accounts_api.php?action=generate_code&account_type_id=" + typeVal)
      ...
      .catch(function (err) {
        ...
        feedback.textContent =
          "An error occurred while generating account code.";
        saveBtn.disabled = true;
      });
  }
    ...
    accountForm.addEventListener("submit", function (e) {
      ...
      if (!isRangeValid || !isCodeUnique) {
        showAlert(
          ...
          "error",
          "Please resolve the invalid fields before saving.",
        );
        return;
      }
      ...
      if (!account_type_id || !account_code || !account_name) {
        showAlert(
          formAlertPlaceholder,
          "error",
          "Account Type, Code, and Name are required fields.",
        );
        return;
      }
