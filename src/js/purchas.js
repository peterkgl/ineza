document.addEventListener("DOMContentLoaded", function () {
  ...
  function renderPurchases() {
    ...
    filtered.forEach(function (p, index) {
      ...
      actionBtns += '<a href="index.php?action=invoice&id=' + p.id + '" target="_blank" class="btn-icon-only" title="View Invoice" style="display: inline-flex; align-items: center; justify-content: center; text-decoration: none; color: inherit; padding: 4px;">' +
        '<svg viewBox="0 0 24 24" style="fill: none; stroke: currentColor; stroke-width: 2; width: 16px; height: 16px;"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/></svg>' +
        '</a>';
  ...
  function showPurchaseDetails(p) {
    ...
    var html = '<div class="detail-modal-list">' +
      ...
      '<div class="detail-modal-item"><div class="detail-modal-label">Purchase Currency</div><div class="detail-modal-val">' + escapeHtml(p.currency_code || 'USD') + '</div></div>' +
      '<div class="detail-modal-item"><div class="detail-modal-label">Amount in Selected Currency</div><div class="detail-modal-val">' + (p.purchase_amount_in_currency ? parseFloat(p.purchase_amount_in_currency).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 4 }) + ' ' + escapeHtml(p.currency_code || 'USD') : '—') + '</div></div>' +
      '<div class="detail-modal-item"><div class="detail-modal-label">Converted Reference Amount</div><div class="detail-modal-val">' + (p.converted_amount ? parseFloat(p.converted_amount).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 4 }) + ' ' + (p.currency_code === 'RWF' ? 'USD' : 'RWF') : '—') + '</div></div>' +
      ...
      '<a href="index.php?action=invoice&id=' + p.id + '" target="_blank" class="btn-sm btn-primary" style="text-decoration: none; display: inline-flex; align-items: center; gap: 6px; padding: 6px 12px; font-size: 12px; font-weight: 500; background-color: var(--primary); color: #fff; border-radius: 4px; border: none; cursor: pointer;">' +
      '<svg viewBox="0 0 24 24" style="width: 14px; height: 14px; fill: none; stroke: currentColor; stroke-width: 2; margin-right: 4px;"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/></svg>' +
      'View Invoice' +
      '</a>' +
      '</div>' +
      '</div>';
